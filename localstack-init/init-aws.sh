...
# Topic: carddemo-batch-alerts
# Purpose: Dedicated SNS topic for batch pipeline failure alerts (AAP §0.3.1).
# Used by: com.cardemo.batch.jobs.BatchPipelineOrchestrator.pipelineListener()
# The application also calls createTopic() idempotently at publish time, so this
# pre-creation is not strictly required — it is included here so that operators
# can see the topic via `awslocal sns list-topics` immediately after init.
echo "Creating SNS topic: carddemo-batch-alerts"
awslocal sns create-topic --name carddemo-batch-alerts
