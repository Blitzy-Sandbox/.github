...
public class AwsConfig {
    ...
    // -------------------------------------------------------------------------
    // SNS Topic Name (from carddemo.aws.sns.* application properties)
    // -------------------------------------------------------------------------
    // SNS topic for batch pipeline alert notifications (5-stage pipeline:
    // POSTTRAN → INTCALC → COMBTRAN → CREASTMT/TRANREPT). Published to by
    // BatchPipelineOrchestrator.pipelineListener.afterJob() when a job ends
    // with BatchStatus.FAILED. Resolves to real AWS SNS topics in production
    // and LocalStack topics in local/test profiles per AAP §0.3.1.
    // -------------------------------------------------------------------------

    @Value("${carddemo.aws.sns.batch-alerts-topic:carddemo-batch-alerts}")
    private String batchAlertsTopic;

    // -------------------------------------------------------------------------
    // AWS SDK Client Override Configuration (from carddemo.aws.client.* properties)
    // -------------------------------------------------------------------------
    // These values govern timeout and retry behavior applied to all three AWS
    // client beans (S3, SQS, SNS). Without explicit overrides, the SDK falls
    // back to defaults that can cause outbound calls to hang for up to 120
    // seconds during AWS outages (observed in QA Phase 3 — Issue #1), which
    // causes Kubernetes readiness probes to flap. Explicit, bounded values
    // ensure health checks report DOWN promptly and retry cascades terminate
    // within a predictable window.
    //
    // Default values (can be overridden via application*.yml):
    //   apiCallTimeoutSeconds        — 15s end-to-end per API call (including retries)
    //   apiCallAttemptTimeoutSeconds —  5s per individual attempt
    //   maxRetries                   —  3 retry attempts (AWS SDK STANDARD mode default)
    // -------------------------------------------------------------------------

    @Value("${carddemo.aws.client.api-call-timeout-seconds:15}")
    private long apiCallTimeoutSeconds;

    @Value("${carddemo.aws.client.api-call-attempt-timeout-seconds:5}")
    private long apiCallAttemptTimeoutSeconds;

    @Value("${carddemo.aws.client.max-retries:3}")
    ...
    /**
     * Builds the shared {@link ClientOverrideConfiguration} applied to all AWS
     * client beans (S3, SQS, SNS) to enforce explicit timeout and retry bounds.
     *
     * <p><strong>Rationale — QA Finding Issue #1 (MINOR):</strong> QA Phase 3
     * testing observed that without explicit {@code ClientOverrideConfiguration},
     * the AWS SDK v2 uses default timeouts that allow outbound calls to hang
     * for up to ~120 seconds (3 attempts × ~40s per-attempt) during AWS
     * outages. This causes the Kubernetes readiness probe backing the
     * {@code /actuator/health} endpoint to hang for the same duration, which
     * in turn causes load balancers to mark the instance unhealthy and
     * producers a production-unsuitable flap pattern.
     *
     * <p>Applying the same {@code ClientOverrideConfiguration} instance to all
     * three clients ensures consistent timeout semantics across the S3 (batch
     * file staging), SQS (report submission), and SNS (batch alert
     * notification) integration boundaries.
     *
     * <p><strong>Retry Strategy:</strong> The {@code STANDARD} retry strategy
     * is used (via {@link DefaultRetryStrategy#standardStrategyBuilder()})
     * instead of the legacy mode to obtain cross-service retry quota
     * coordination and adaptive retry behavior. The total number of attempts
     * is capped at {@code maxRetries + 1} — one initial attempt plus
     * {@link #maxRetries} retries (default 3, yielding 4 total attempts) —
     * so the total time envelope for any failing call is bounded by
     * {@code apiCallTimeoutSeconds}.
     *
     * <p><strong>API migration note:</strong> AWS SDK v2 deprecated the
     * {@code RetryPolicy} API in favor of the newer {@code RetryStrategy}
     * API under {@code software.amazon.awssdk.retries}. This implementation
     * uses the non-deprecated {@link DefaultRetryStrategy} factory to build
     * a {@code StandardRetryStrategy} with an explicit {@code maxAttempts}
     * cap, avoiding {@code -Werror} compile failures from deprecation
     * warnings.
     *
     * @return a {@link ClientOverrideConfiguration} configured with explicit
     *         API-call timeout, per-attempt timeout, and retry strategy
     */
    private ClientOverrideConfiguration defaultOverrideConfiguration() {
        return ClientOverrideConfiguration.builder()
                ...
                // maxAttempts = initial attempt + retries. With maxRetries=3 (default),
                // this yields 4 total attempts (1 initial + 3 retries), matching the
                // semantics of the deprecated RetryPolicy.builder().numRetries(3).
    ...
    public S3Client s3Client() {
        var builder = S3Client.builder()
                ...
                // Apply explicit API-call timeout, per-attempt timeout, and retry policy
                // to bound outbound-call duration and prevent health-check hangs during
                // AWS outages (QA Finding Issue #1 — MINOR).
        ...
        log.info("AWS S3 client initialized — region: {}, pathStyleAccess: {}, buckets: [{}, {}, {}], "
                        + "apiCallTimeout: {}s, apiCallAttemptTimeout: {}s, maxRetries: {}",
                awsRegion, s3PathStyleAccessEnabled,
                batchInputBucket, batchOutputBucket, statementsBucket,
                apiCallTimeoutSeconds, apiCallAttemptTimeoutSeconds, maxRetries);
    ...
    public SqsClient sqsClient() {
        ...
        log.info("AWS SQS client initialized — region: {}, reportQueue: {}, "
                        + "apiCallTimeout: {}s, apiCallAttemptTimeout: {}s, maxRetries: {}",
                awsRegion, reportQueue,
                apiCallTimeoutSeconds, apiCallAttemptTimeoutSeconds, maxRetries);
    ...
    public SnsClient snsClient() {
        ...
        log.info("AWS SNS client initialized — region: {}, batchAlertsTopic: {}, "
                        + "apiCallTimeout: {}s, apiCallAttemptTimeout: {}s, maxRetries: {}",
                awsRegion, batchAlertsTopic,
                apiCallTimeoutSeconds, apiCallAttemptTimeoutSeconds, maxRetries);
    ...
    /**
     * Returns the SNS topic name for batch pipeline alert notifications.
     *
     * <p>Used by {@code BatchPipelineOrchestrator.pipelineListener} to publish
     * alert messages when a Spring Batch job terminates with
     * {@code BatchStatus.FAILED}. Resolves to a real AWS SNS topic in
     * production and a LocalStack-created topic in local/test profiles
     * (AAP §0.3.1). Default: {@code carddemo-batch-alerts}.
     *
     * @return the batch alerts SNS topic name
     */
    public String getBatchAlertsTopic() {
        return batchAlertsTopic;
    }
}
