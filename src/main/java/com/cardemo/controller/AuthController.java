...
import com.cardemo.service.interfaces.AuthService;
 ...
 * The controller delegates to {@link AuthService} which performs
 ...
 *
 * @see com.cardemo.service.interfaces.AuthService
 * @see SignOnRequest
 * @see SignOnResponse
 */
@RestController
...
public class AuthController {
    ...
    /**
     * Authentication service contract providing USRSEC lookup and BCrypt verification.
     * Spring injects the concrete implementation from {@code com.cardemo.service.auth}
     * at runtime; the controller depends only on the interface.
     *
     * <p>Migrated from COBOL program COSGN00C.cbl (260 lines, CICS transaction CC00).
     * Performs: uppercase normalization → USRSEC repository lookup → BCrypt password
     * verification → routing metadata construction (ADMIN→COADM01C, USER→COMEN01C).</p>
     */
     ...
     *   <li>Delegate to {@link AuthService#authenticate(SignOnRequest)}</li>
    ...
    public ResponseEntity<SignOnResponse> signIn(@Valid @RequestBody SignOnRequest request) {
        ...
        try {
            SignOnResponse response = authService.authenticate(request);
