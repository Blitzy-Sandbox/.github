 ...
 * criteria from the client and delegates to {@link ReportService}, which
 ...
 *       <td>{@link ReportService#submitReport(ReportRequest)} — SQS publish</td></tr>
 ...
 *
 * @see com.cardemo.service.interfaces.ReportService
 * @see ReportRequest
 */
@RestController
...
public class ReportController {
    ...
    /**
     * Report submission service contract implementing the online-to-batch bridge.
     * Spring injects the concrete implementation from {@code com.cardemo.service.report}
     * at runtime; the controller depends only on the interface.
     *
     * <p>Validates report criteria, calculates date ranges, and publishes to SQS FIFO queue
     * ({@code carddemo-report-jobs.fifo}) replacing the original CICS Transient Data Queue
     * (TDQ) WRITEQ TD QUEUE('JOBS'). Injected via constructor injection (no @Autowired
     * annotation per Spring best practices).</p>
     */
    private final ReportService reportService;

    /**
     * Constructs a new ReportController with the required ReportService dependency.
     *
     ...
     *
     * @param reportService the service contract handling report criteria validation
