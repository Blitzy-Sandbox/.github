 ...
 *
 * @see com.cardemo.service.interfaces.AccountService
 * @see AccountDto
 */
@RestController
...
public class AccountController {
    ...
    /**
     * Account domain service contract aggregating view and update operations.
     * Spring injects the concrete implementation(s) in {@code com.cardemo.service.account}
     * at runtime; the controller depends only on the interface.
     *
     * <p>Covers:</p>
     * <ul>
     *   <li><strong>COACTVWC.cbl (941 lines)</strong> — Account view with multi-dataset
     *       read chain: CardCrossReference (CXACAIX) → Account (ACCTDAT) → Customer (CUSTDAT).</li>
     *   <li><strong>COACTUPC.cbl (4,236 lines — most complex program)</strong> — Dual-dataset
     *       atomic update with SYNCPOINT semantics: 25+ field validations → Account + Customer
     *       atomic update with {@code @Transactional(rollbackFor = Exception.class)} and JPA
    ...
    /**
     * Constructs a new AccountController with the account domain service dependency.
     *
     ...
     *
     * @param accountService service contract for account view and update operations
     *                       (COACTVWC.cbl + COACTUPC.cbl)
     */
    ...
    public ResponseEntity<AccountDto> getAccount(@PathVariable String id) {
        ...
        AccountDto result = accountService.getAccountView(id);
