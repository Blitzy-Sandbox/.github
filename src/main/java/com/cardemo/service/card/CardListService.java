...
public class CardListService implements CardService {
    ...
    public Page<CardDto> listCards(int page, String acctIdFilter, String cardNumFilter) {
        ...
        if (!hasAcctFilter && !hasCardFilter) {
            ...
            Pageable pageable = PageRequest.of(page, PaginationConstants.CARD_PAGE_SIZE,
    ...
    private Page<CardDto> paginateCards(List<Card> cards, int page) {
        ...
        int start = page * PaginationConstants.CARD_PAGE_SIZE;
        int end = Math.min(start + PaginationConstants.CARD_PAGE_SIZE, totalElements);
    ...
    private void validateAccountIdFilter(String acctIdFilter) {
        ...
        if (trimmed.length() > CardConstants.ACCT_ID_MAX_LENGTH) {
            throw new IllegalArgumentException(
                    "Account filter, if supplied, must be at most "
                            + CardConstants.ACCT_ID_MAX_LENGTH + " digits");
        }
    ...
    private void validateCardNumFilter(String cardNumFilter) {
        ...
        if (trimmed.length() > CardConstants.CARD_NUM_MAX_LENGTH) {
            throw new IllegalArgumentException(
                    "Card ID filter, if supplied, must be at most "
                            + CardConstants.CARD_NUM_MAX_LENGTH + " digits");
        }
        if (!trimmed.matches("\\d+")) {
            throw new IllegalArgumentException(
    ...
    private String maskAccountId(String acctId) {
        ...
        if (trimmed.length() <= CardConstants.MASK_VISIBLE_DIGITS) {
            return "***" + trimmed;
        }
        return "***" + trimmed.substring(trimmed.length() - CardConstants.MASK_VISIBLE_DIGITS);
    }
    ...
    // CardListService owns only the two list/browse methods above
    // (listCards, listCardsByAccount). The four methods below are declared
    // on the interface but are implemented by the sibling @Service beans
    // (CardDetailService, CardUpdateService) and therefore throw
    // UnsupportedOperationException when invoked on this instance. This
    // pattern matches the one used by CardDetailService and the
    // AdminService / MenuService / TransactionService aggregated
    // interfaces per the folder-level AAP Section 0.5.1
    // ("Interface-Contract Stubs" pattern).

    /**
     * Not supported by {@code CardListService}. Implemented by
    ...
    public CardDto getCardDetail(String cardNum) {
        throw new UnsupportedOperationException(
                "getCardDetail is implemented by CardDetailService, not CardListService");
    }
     ...
     * {@code CardDetailService} bean directly rather than this list
     * service instance. Note that {@link #listCardsByAccount(String, int)}
     * on this class provides a PAGINATED variant of account-scoped card
     * browse; the stubbed method is the non-paginated variant.</p>
     *
    ...
    public List<CardDto> getCardsByAccountId(String acctId) {
        throw new UnsupportedOperationException(
                "getCardsByAccountId is implemented by CardDetailService, not CardListService");
    }

    /**
     * Not supported by {@code CardListService}. Implemented by
     * {@code com.cardemo.service.card.CardUpdateService#getCardForUpdate(String)}.
     *
     * <p>Consumers that need the read-for-update pre-fetch should inject
     * the {@code CardUpdateService} bean directly rather than this list
     * service instance.</p>
     *
     * @param cardNum ignored
     * @return never returns normally
    ...
    public CardDto getCardForUpdate(String cardNum) {
        throw new UnsupportedOperationException(
                "getCardForUpdate is implemented by CardUpdateService, not CardListService");
    }

    /**
     * Not supported by {@code CardListService}. Implemented by
     * {@code com.cardemo.service.card.CardUpdateService#updateCard(String, CardDto)}.
     *
     * <p>Consumers that need the optimistic-locking card update operation
     * should inject the {@code CardUpdateService} bean directly rather
     * than this list service instance.</p>
     *
     * @param cardNum       ignored
     * @param updateRequest ignored
     * @return never returns normally
     * @throws UnsupportedOperationException always
     */
    @Override
    public CardDto updateCard(String cardNum, CardDto updateRequest) {
        throw new UnsupportedOperationException(
                "updateCard is implemented by CardUpdateService, not CardListService");
    }
}
