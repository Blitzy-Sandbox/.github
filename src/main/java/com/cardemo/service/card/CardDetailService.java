...
public class CardDetailService implements CardService {
    ...
    private void validateCardNumber(String cardNum) {
        ...
        if (cardNum.length() > CardConstants.CARD_NUM_MAX_LENGTH) {
            throw new IllegalArgumentException(
                    "Card number must not exceed " + CardConstants.CARD_NUM_MAX_LENGTH + " characters");
        }
    ...
    private void validateAccountId(String acctId) {
        ...
        if (acctId.length() > CardConstants.ACCT_ID_MAX_LENGTH) {
            throw new IllegalArgumentException(
                    "Account ID must not exceed " + CardConstants.ACCT_ID_MAX_LENGTH + " characters");
        }
    ...
    // CardDetailService owns only the two detail-lookup methods above. The
    // four methods below are declared on the interface but are implemented
    // by the sibling @Service beans and therefore throw
    ...
    // TransactionService aggregated interfaces per the folder-level AAP
    // ("Interface-Contract Stubs" pattern).

    /**
     * Not supported by {@code CardDetailService}. Implemented by
     * {@code com.cardemo.service.card.CardListService#listCards(int, String, String)}.
     *
     * <p>Consumers that need the paginated card browse operation should inject
     * the {@code CardListService} bean directly (or the {@link CardService}
     * interface qualified to that bean) rather than this detail service
     * instance.</p>
     *
     * @param page          ignored
    ...
    public Page<CardDto> listCards(int page, String acctIdFilter, String cardNumFilter) {
        throw new UnsupportedOperationException(
                "listCards is implemented by CardListService, not CardDetailService");
    }
     ...
     * rather than this detail service instance.</p>
     *
    ...
    public Page<CardDto> listCardsByAccount(String acctId, int page) {
        throw new UnsupportedOperationException(
                "listCardsByAccount is implemented by CardListService, not CardDetailService");
    }
     ...
     *
     * <p>Consumers that need the read-for-update pre-fetch should inject the
     * {@code CardUpdateService} bean directly rather than this detail service
     * instance.</p>
     *
    ...
    public CardDto getCardForUpdate(String cardNum) {
        throw new UnsupportedOperationException(
                "getCardForUpdate is implemented by CardUpdateService, not CardDetailService");
    }
     ...
     * should inject the {@code CardUpdateService} bean directly rather than
     * this detail service instance.</p>
     *
     * @param cardNum       ignored
    ...
    public CardDto updateCard(String cardNum, CardDto updateRequest) {
        throw new UnsupportedOperationException(
                "updateCard is implemented by CardUpdateService, not CardDetailService");
    }
}
