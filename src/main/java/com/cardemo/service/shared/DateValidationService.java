 ...
 *
 * <p>Post-refactoring note: The seven date-validation support constants
 * previously declared locally (MONTHS_WITH_31_DAYS, FEBRUARY, LAST_CENTURY,
 * THIS_CENTURY, CCYYMMDD_LENGTH, CCYYMMDD_FORMATTER, CEEDAYS_VALID) have
 * been centralized into {@link DateFormatConstants} per AAP §0.4.3
 * (Constant Holder Pattern) and §0.7.4 (Constant Duplication Impact
 * Analysis). Semantic behavior is byte-identical; only the declaration
 * location changed.
 *
...
public class DateValidationService {
    ...
    public DateValidationResult validateDate(String dateStr, String variableName) {
        ...
        // ────────────────────────────────────────────────────────────────────
        if (dateStr == null || dateStr.isBlank() || dateStr.length() != DateFormatConstants.CCYYMMDD_LENGTH) {
        ...
        if (ccNum != DateFormatConstants.THIS_CENTURY && ccNum != DateFormatConstants.LAST_CENTURY) {
        ...
        if (!DateFormatConstants.MONTHS_WITH_31_DAYS.contains(monthNum) && dayNum == 31) {
        ...
        if (monthNum == DateFormatConstants.FEBRUARY && dayNum == 30) {
        ...
        if (monthNum == DateFormatConstants.FEBRUARY && dayNum == 29) {
        ...
        // ────────────────────────────────────────────────────────────────────
        try {
            LocalDate.parse(dateStr, DateFormatConstants.CCYYMMDD_STRICT_FORMATTER);
        ...
        // ────────────────────────────────────────────────────────────────────
        String fullMessage = buildCeedaysMessage(0, 0, DateFormatConstants.CEEDAYS_VALID, dateStr, "YYYYMMDD");
        logger.info("Date validation successful: dateStr='{}'", dateStr);
        return new DateValidationResult(true, 0, DateFormatConstants.CEEDAYS_VALID, fullMessage,
                true, true, true);
    }
    ...
    public DateValidationResult validateDateOfBirth(String dateStr, String variableName) {
        ...
        LocalDate editDate = LocalDate.parse(dateStr, DateFormatConstants.CCYYMMDD_STRICT_FORMATTER);
    ...
    public DateValidationResult validateWithCeedays(String dateStr, String formatMask) {
        ...
        String fullMsg = buildCeedaysMessage(0, 0, DateFormatConstants.CEEDAYS_VALID,
                dateTrimmed, maskTrimmed);
        logger.info("CEEDAYS validation successful: dateStr='{}', formatMask='{}'",
                dateTrimmed, maskTrimmed);
        return new DateValidationResult(true, 0, DateFormatConstants.CEEDAYS_VALID, fullMsg,
                true, true, true);
    }
