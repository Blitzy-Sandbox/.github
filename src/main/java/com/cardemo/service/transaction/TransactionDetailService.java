 ...
 * <p>This service implements the shared {@link TransactionService} contract, which
 * aggregates the six transaction-domain operations across the three concrete services
 * ({@code TransactionAddService}, {@code TransactionDetailService},
 * {@code TransactionListService}). This class owns
 * {@link #getTransaction(String)} only; the remaining five interface methods throw
 * {@link UnsupportedOperationException} because they are owned by
 * {@code TransactionAddService} and {@code TransactionListService}. Controllers
 * continue to inject the concrete {@code TransactionDetailService} bean for detail
 * use-cases; callers that need the full interface contract inject the {@code @Primary}
 * {@code TransactionServiceImpl} composite, which delegates to each owning service.
 * This stub pattern is documented in AAP §0.7.3 and matches the approach used for
 * other aggregated service interfaces (AdminService, CardService, MenuService).</p>
 *
 * @see Transaction
 * @see TransactionDto
 ...
 */
@Service
public class TransactionDetailService implements TransactionService {
    ...
    // owns only getTransaction(). The remaining five interface methods below
    // throw UnsupportedOperationException because they are owned by other
    // services. Controllers continue to inject the concrete class for detail
    // use-cases; callers that need the full interface contract inject the
    // @Primary TransactionServiceImpl composite, which delegates to each
    // owning service.
    //
    ...
    /**
     * Not supported by {@code TransactionDetailService}. Implemented by
     * {@link com.cardemo.service.transaction.TransactionAddService#addTransaction(TransactionDto)}
     * (COTRN02C.cbl, CT02 — "Add a Transaction").
     *
     * <p>Callers that require {@code addTransaction} via the shared
     * {@link TransactionService} contract should inject the
     * {@code @Primary} {@code TransactionServiceImpl} composite, which
     * delegates to {@code TransactionAddService}.</p>
     *
     * @param request the transaction payload (ignored by this stub)
    ...
    public TransactionDto addTransaction(TransactionDto request) {
        throw new UnsupportedOperationException(
                "addTransaction is implemented by TransactionAddService, not TransactionDetailService");
    }

    /**
     * Not supported by {@code TransactionDetailService}. Implemented by
     * {@link com.cardemo.service.transaction.TransactionAddService#copyFromTransaction(String)}
     * (COTRN02C.cbl, CT02 — "Copy from a Transaction").
     *
     * <p>Callers that require {@code copyFromTransaction} via the shared
     * {@link TransactionService} contract should inject the
     * {@code @Primary} {@code TransactionServiceImpl} composite, which
     * delegates to {@code TransactionAddService}.</p>
     *
     * @param sourceTransactionId the source transaction ID (ignored by this stub)
     * @return never returns normally
     * @throws UnsupportedOperationException always
     */
    @Override
    public TransactionDto copyFromTransaction(String sourceTransactionId) {
        throw new UnsupportedOperationException(
                "copyFromTransaction is implemented by TransactionAddService, not TransactionDetailService");
    }
    ...
    public Page<TransactionDto> listTransactions(String startTransactionId, int page) {
        throw new UnsupportedOperationException(
                "listTransactions is implemented by TransactionListService, not TransactionDetailService");
    }
    ...
    public boolean hasNextPage(Page<TransactionDto> currentPage) {
        throw new UnsupportedOperationException(
                "hasNextPage is implemented by TransactionListService, not TransactionDetailService");
    }
    ...
    public TransactionListService.PageNavigation getPageNavigation(Page<TransactionDto> page) {
        throw new UnsupportedOperationException(
                "getPageNavigation is implemented by TransactionListService, not TransactionDetailService");
    }
}
