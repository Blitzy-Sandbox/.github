 ...
 */
@Service
public class BillPaymentService implements BillingService {
    ...
    /**
     * Domain rule component for credit-limit validation.
     * Per AAP Section 0.4.3 (Domain Rules Pattern): stateless, framework-independent
     * business-rule class encapsulating credit-limit checking logic previously inlined
     * in this service (COBIL00C.cbl paragraph PROCESS-ENTER-KEY, AAP credit-limit validation).
     */
     ...
     * @param creditLimitRules              domain rule component delegating credit-limit validation
     *                                       (AAP Section 0.4.3 — Domain Rules Pattern)
     */
    public BillPaymentService(AccountRepository accountRepository,
                              TransactionRepository transactionRepository,
                              CardCrossReferenceRepository cardCrossReferenceRepository,
                              MetricsConfig metricsConfig,
                              CreditLimitRules creditLimitRules) {
    ...
    public TransactionDto processPayment(String accountId) {
        ...
        // Delegated to domain rule class CreditLimitRules per AAP Section 0.4.3
        // (Domain Rules Pattern). The rule throws CreditLimitExceededException with the
        // canonical 4-arg pattern where transactionAmount == currentBalance, preserving
        // pre-refactoring BillPaymentService behavior exactly (AAP Rule R-001).
        //
        // @see com.cardemo.domain.rules.CreditLimitRules#ensureWithinOnlineCreditLimit
        creditLimitRules.ensureWithinOnlineCreditLimit(
                accountId, currentBalance, account.getAcctCreditLimit());
    ...
    private String generateTransactionId() {
        ...
        long nextId = maxId.map(id -> {
            try {
                ...
                return TransactionConstants.DEFAULT_STARTING_ID;
            }
        }).orElse(TransactionConstants.DEFAULT_STARTING_ID);

        String transactionId = String.format(TransactionConstants.TRANSACTION_ID_FORMAT, nextId);
