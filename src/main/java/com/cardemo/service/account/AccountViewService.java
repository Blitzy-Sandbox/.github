 ...
 */
@Service
public class AccountViewService implements AccountService {
    ...
    /**
     * Not applicable to the read-only account view service. The account fetch for
     * the update flow (which exposes the JPA version token for optimistic locking)
     * is served by {@link com.cardemo.service.account.AccountUpdateService#getAccount(String)}.
     *
     ...
     * this service throws {@link UnsupportedOperationException}.</p>
     *
    ...
    public AccountDto getAccount(String acctId) {
        throw new UnsupportedOperationException(
                "getAccount is handled by AccountUpdateService, not AccountViewService");
    }

    /**
     * Not applicable to the read-only account view service. Account updates (with
     * validation, optimistic locking, and transactional persistence) are handled by
     * {@link com.cardemo.service.account.AccountUpdateService#updateAccount(String, AccountDto)}.
     *
     * <p>This method exists solely to satisfy the {@link AccountService} interface contract
     * per the aggregated-interface pattern documented in AAP Section 0.7.3. Calling it on
     * this service throws {@link UnsupportedOperationException}.</p>
     *
     * @param acctId      the 11-digit account identifier (not used)
     * @param updatedData the account update DTO (not used)
     * @return never returns
     * @throws UnsupportedOperationException always
     */
    @Override
    public AccountDto updateAccount(String acctId, AccountDto updatedData) {
        throw new UnsupportedOperationException(
                "updateAccount is handled by AccountUpdateService, not AccountViewService");
    }
    ...
    private void validateAccountId(String acctId) {
        ...
        if (acctId.length() != AccountConstants.ACCOUNT_ID_LENGTH) {
            logger.debug("Account ID validation failed: length {} (expected {})",
                    acctId.length(), AccountConstants.ACCOUNT_ID_LENGTH);
            throw new IllegalArgumentException(
                    "Account Number if supplied must be a 11 digit Non-Zero Number");
        }
        ...
        if (AccountConstants.ALL_ZEROS_ACCOUNT_ID.equals(acctId)) {
