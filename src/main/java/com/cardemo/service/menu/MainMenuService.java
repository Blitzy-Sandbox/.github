...
public class MainMenuService implements MenuService {
    ...
    /**
     * Interface-contract stub. {@code MainMenuService} does not provide
     * admin-menu options — see
     * {@link com.cardemo.service.menu.AdminMenuService#getAdminMenuOptions()}
     * for the admin menu implementation.
     *
     ...
     * architecture, {@code COMEN01C} (main menu, regular users) and {@code COADM01C}
     * (admin menu) were separate CICS programs with non-overlapping responsibilities;
     * this separation is preserved in Java by routing admin-menu calls to
     * {@code AdminMenuService}.</p>
     *
    ...
    public List<AdminMenuService.AdminMenuOption> getAdminMenuOptions() {
        throw new UnsupportedOperationException(
                "MainMenuService does not provide admin menu options; use AdminMenuService.getAdminMenuOptions()");
    }

    /**
     * Interface-contract stub. {@code MainMenuService} resolves main options via
     * {@link #getMenuOption(int)} — see
     * {@link com.cardemo.service.menu.AdminMenuService#getAdminMenuOption(int)} for
     * the admin menu implementation.
     *
     ...
     */
    @Override
    public AdminMenuService.AdminMenuOption getAdminMenuOption(int optionNumber) {
        throw new UnsupportedOperationException(
                "MainMenuService does not resolve admin menu options; use getMenuOption(int) "
                        + "or AdminMenuService.getAdminMenuOption(int)");
    }

    /**
     * Indicates whether this menu is restricted to admin-type users only.
     *
     * <p>Always returns {@code false} for the main menu. The main menu
     * ({@code COMEN01C.cbl}) is the default landing menu for regular users
     * ({@link UserType#USER}). Per-option access control is still enforced at
     * the option level via {@link #isOptionAccessible(int, UserType)} and
     * {@link #getMenuOptionsForUser(UserType)}, mirroring the
     * {@code COMEN01C.cbl PROCESS-ENTER-KEY} user-type checks (lines 136-143).</p>
     *
     * <p>This is the logical counterpart of
     * {@link com.cardemo.service.menu.AdminMenuService#isAdminOnly()}, which
     * always returns {@code true}.</p>
     *
     * @return {@code false} always — the main menu is accessible to regular users
     */
    @Override
