...
public class AdminMenuService implements MenuService {
    ...
    /**
     * Interface-contract stub. {@code AdminMenuService} does not provide
     * main-menu options — see {@link com.cardemo.service.menu.MainMenuService#getMenuOptions()}
     * for the main menu implementation.
     *
     * <p>This stub exists solely to satisfy the {@link MenuService} interface
     * contract per the refactoring that introduces a shared menu-service interface
     * across {@code MainMenuService} and {@code AdminMenuService}. In the COBOL
     * architecture, {@code COADM01C} (admin menu) and {@code COMEN01C} (main menu)
     * were separate CICS programs with non-overlapping responsibilities; this
     * separation is preserved in Java by routing main-menu calls to
     * {@code MainMenuService}.</p>
     *
    ...
    public List<MainMenuService.MenuOption> getMenuOptions() {
        throw new UnsupportedOperationException(
                "AdminMenuService does not provide main menu options; use MainMenuService.getMenuOptions()");
    }

    /**
     * Interface-contract stub. {@code AdminMenuService} does not filter main-menu
     * options by user type — see
     * {@link com.cardemo.service.menu.MainMenuService#getMenuOptionsForUser(UserType)}
     * for the main menu implementation. The admin menu itself is restricted to
     * admin users at the security layer; see {@link #isAdminOnly()}.
     *
     * @param userType unused
     * @return this method never returns normally
     * @throws UnsupportedOperationException always
     */
    @Override
    public List<MainMenuService.MenuOption> getMenuOptionsForUser(UserType userType) {
        throw new UnsupportedOperationException(
                "AdminMenuService does not provide main menu options; use MainMenuService.getMenuOptionsForUser(UserType)");
    }

    /**
     * Interface-contract stub. {@code AdminMenuService} resolves admin options via
     * {@link #getAdminMenuOption(int)} — see
     * {@link com.cardemo.service.menu.MainMenuService#getMenuOption(int)} for the
     * main menu implementation.
     *
     * @param optionNumber unused
     * @return this method never returns normally
     * @throws UnsupportedOperationException always
     */
    @Override
    public MainMenuService.MenuOption getMenuOption(int optionNumber) {
        throw new UnsupportedOperationException(
                "AdminMenuService does not resolve main menu options; use getAdminMenuOption(int) "
                        + "or MainMenuService.getMenuOption(int)");
    }

    /**
     * Interface-contract stub. Access control for admin menu entries is inherent
     * to the menu itself (see {@link #isAdminOnly()}); there is no per-option
     * user-type filter for admin menu items because the admin options table
     * ({@code COADM02Y.cpy}) does not include a user type column. For main menu
     * access checks, see
     * {@link com.cardemo.service.menu.MainMenuService#isOptionAccessible(int, UserType)}.
     *
     * @param optionNumber unused
     * @param userType     unused
     * @return this method never returns normally
    ...
    public boolean isOptionAccessible(int optionNumber, UserType userType) {
        throw new UnsupportedOperationException(
                "AdminMenuService does not perform per-option access checks; use isAdminOnly() "
                        + "or MainMenuService.isOptionAccessible(int, UserType)");
    }
}
