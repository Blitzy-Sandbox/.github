...
import com.cardemo.domain.rules.TransactionPostingRules;
...
public class TransactionPostingProcessor implements ItemProcessor<DailyTransaction, Transaction> {
    ...
    /**
     * Domain rules delegate encapsulating the 4-stage validation cascade logic
     * extracted from this processor per AAP §0.4.3 (Domain Rules Pattern) and
     * §0.7.5 (Batch Pipeline State Dependency Graph — safe extraction).
     *
     * <p>Maps to COBOL paragraph {@code 1500-VALIDATE-TRAN} which orchestrates
     * 1500-A-LOOKUP-XREF, 1500-B-LOOKUP-ACCT, the credit limit check, and the
     * expiry check. The cascade short-circuits on the first failure in COBOL
     * sequential order, preserving byte-identical validation outcomes.</p>
     *
     * <p>The processor performs the XREF and ACCOUNT repository lookups eagerly
     * and passes the resulting {@link Optional} values into the rules class,
     * which keeps infrastructure (JPA) and pure business logic (validation)
     * decoupled per the layered architecture defined in AAP §0.4.1.</p>
     */
    private final TransactionPostingRules postingRules;
     ...
     * @param postingRules domain rules delegate encapsulating the 4-stage
     *        validation cascade (XREF lookup, account lookup, credit limit
     *        check, expiry check); extracted from this processor per
     *        AAP §0.4.3 and §0.7.5. Must not be {@code null}.
     */
    public TransactionPostingProcessor(
            ...
            TransactionPostingRules postingRules) {
    ...
    public Transaction process(DailyTransaction item) throws Exception {
        ...
        // ---------------------------------------------------------------
        // 4-Stage Validation Cascade — delegated to TransactionPostingRules
        // Maps to COBOL paragraph 1500-VALIDATE-TRAN (CBTRN02C.cbl) which
        // sequences: 1500-A-LOOKUP-XREF (lines 380-392, reject code 100),
        // 1500-B-LOOKUP-ACCT (lines 393-402, reject code 101),
        // credit limit check (lines 403-413, reject code 102), and
        // expiry check (lines 414-420, reject code 103).
        //
        // The processor performs repository lookups eagerly to produce
        // Optional<CardCrossReference> and Optional<Account> inputs for
        // the domain rules class. The rules class encapsulates the pure
        // validation logic (short-circuiting on first failure in COBOL
        // sequential order) while infrastructure (JPA) stays here.
        //
        // Note: "EXPIRAION" (CBTRN02C.cbl line 414) is the original COBOL
        // field name (typo preserved in source). The Java domain uses the
        // corrected name acctExpDate; the rules class handles the
        // LocalDateTime -> LocalDate truncation internally.
        // ---------------------------------------------------------------
        Optional<CardCrossReference> xrefOpt =
                cardCrossReferenceRepository.findById(item.getDalytranCardNum());
        Optional<Account> acctOpt = xrefOpt
                .map(CardCrossReference::getXrefAcctId)
                .flatMap(accountRepository::findById);

        TransactionPostingRules.ValidationResult result =
                postingRules.validateCascade(item, xrefOpt, acctOpt);
        if (!result.approved()) {
            rejectTransaction(item, result.rejectCode());
            return null;
        }
    ...
    private boolean updateAccount(Account account, DailyTransaction item) {
        ...
        // Delegates the amount-sign classification to TransactionPostingRules
        // per AAP §0.7.5 extraction-safety matrix (pure predicate, stateless).
        if (postingRules.isCreditCycleAmount(amount)) {
