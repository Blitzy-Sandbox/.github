...
7. [Domain Context — CardDemo Business Overview](#7-domain-context--carddemo-business-overview)
   - [7.6 Layered Architecture — Post-Refactoring Package Organization](#76-layered-architecture--post-refactoring-package-organization)
...
### 7.6 Layered Architecture — Post-Refactoring Package Organization

CardDemo follows a strict **layered domain-driven architecture** after the post-migration refactoring (see [DECISION_LOG.md](../DECISION_LOG.md) D-019). Understanding this package organization is essential before writing any new code.

#### 7.6.1 Package Layers at a Glance

```
com.cardemo/
├── controller/                 ← Spring MVC REST controllers (8 classes)
├── service/
│   ├── interfaces/             ← Service contracts (8 interfaces) ← INJECT THIS TYPE
│   ├── account/                ← Account service implementations
│   ├── admin/                  ← User administration service implementations
│   ├── auth/                   ← Authentication service implementation
│   ├── billing/                ← Billing service implementation
│   ├── card/                   ← Card service implementations
│   ├── menu/                   ← Menu service implementations
│   ├── report/                 ← Report submission service implementation
│   ├── shared/                 ← Shared utility services (date, file status, lookup)
│   └── transaction/            ← Transaction service implementations
├── domain/                     ← NEW: Framework-independent business logic
│   ├── constants/              ← Shared business constants (Account, Card, Date, etc.)
│   ├── validation/             ← Field-level validators (Account, Card, Transaction, User)
│   └── rules/                  ← Stateless business rules (CreditLimit, Interest, TransactionPosting)
├── repository/                 ← Spring Data JPA repositories (11 interfaces)
├── model/                      ← DTOs, entities, enums, composite keys
├── batch/                      ← Spring Batch jobs, processors, readers, writers
├── config/                     ← Spring configuration classes (decomposed)
│   ├── WebConfig.java          ← CORS + request logging ONLY
│   ├── JacksonConfig.java      ← Jackson ObjectMapper customization ONLY
│   ├── GlobalExceptionHandler.java ← @RestControllerAdvice ONLY (15 @ExceptionHandler methods)
│   └── (other config files)    ← AwsConfig, BatchConfig, JpaConfig, ObservabilityConfig, SecurityConfig
├── exception/                  ← Custom exception hierarchy
└── observability/              ← CorrelationIdFilter, HealthIndicators, MetricsConfig
```

#### 7.6.2 The `domain/` Layer — Framework-Independent Business Logic

The `com.cardemo.domain/` package contains **pure business logic independent of Spring Framework annotations**. This separation enables:

- Fast unit tests that run without a Spring context
- Clear separation of "what the business does" (domain) from "how Spring wires it" (service)
- Reusability of rules across service and batch layers

**Three sub-packages:**

| Sub-Package | Purpose | Example Classes |
|-------------|---------|-----------------|
| `domain/constants/` | **Centralized business constants.** Holds `public static final` fields in `final` classes with private constructors. Eliminates duplication of field lengths, regex patterns, `DateTimeFormatter` instances, and pagination sizes previously scattered across 20+ service files. | `AccountConstants`, `CardConstants`, `DateFormatConstants`, `PaginationConstants`, `TransactionConstants`, `ValidationPatterns` |
| `domain/validation/` | **Field-level validators** extracted from monolithic service classes. Each validator method receives primitive/DTO inputs and throws `ValidationException` with COBOL-parity error messages. Pure Java — no Spring annotations. | `AccountValidator`, `CardValidator`, `TransactionValidator`, `UserValidator` |
| `domain/rules/` | **Stateless business rule calculations** extracted from service and batch processor classes. Encapsulates multi-step domain operations that can be unit-tested without Spring context. | `CreditLimitRules`, `InterestCalculationRules`, `TransactionPostingRules` |

#### 7.6.3 The `service/interfaces/` Package — Service Contracts

Every service domain defines a **public interface** under `com.cardemo.service.interfaces/`. Controllers and batch orchestrators inject the **interface type**, not the concrete implementation.

**The 8 service interfaces:**

| Interface | Covers Implementations |
|-----------|-----------------------|
| `AccountService` | `AccountUpdateService`, `AccountViewService` |
| `AdminService` | `UserAddService`, `UserDeleteService`, `UserListService`, `UserUpdateService` |
| `AuthService` | `AuthenticationService` |
| `BillingService` | `BillPaymentService` |
| `CardService` | `CardDetailService`, `CardListService`, `CardUpdateService` |
| `MenuService` | `AdminMenuService`, `MainMenuService` |
| `ReportService` | `ReportSubmissionService` |
| `TransactionService` | `TransactionAddService`, `TransactionDetailService`, `TransactionListService` |

**Correct injection pattern (controllers):**

```java
// CORRECT — inject the INTERFACE type
@RestController
@RequestMapping("/api/accounts")
public class AccountController {
    private final AccountService accountService;  // ← interface
...
// WRONG — do not inject concrete @Service classes from controllers
private final AccountUpdateService accountUpdateService;  // ← don't do this
```

#### 7.6.4 The Decomposed `config/` Package

The previously monolithic 840-line `WebConfig.java` has been split into three single-responsibility classes. This decomposition ensures that each configuration class has exactly one reason to change.

| Class | Responsibility | Contains |
|-------|----------------|----------|
| `WebConfig.java` | CORS mappings + request logging filter | `addCorsMappings()`, `requestLoggingFilter()` |
| `JacksonConfig.java` | Jackson `ObjectMapper` customization | `Jackson2ObjectMapperBuilderCustomizer` bean |
| `GlobalExceptionHandler.java` | Global error → HTTP response mapping | `@RestControllerAdvice` with 15 `@ExceptionHandler` methods + nested `ErrorResponse` record |

**Key rule:** New exception handler methods go in `GlobalExceptionHandler.java`, never in `WebConfig.java`.

#### 7.6.5 Why This Layered Structure?

The refactoring preserves **100% behavioral parity** with the original COBOL application (no business logic changes) while improving:

- **Maintainability** — Smaller, cohesive classes (no more 980-line services)
- **Testability** — Domain classes unit-test without Spring context (fast)
- **Readability** — Single source of truth for constants (e.g., one `DateFormatConstants.CCYYMMDD_FORMATTER` replaces 11 duplicates)
- **Separation of concerns** — Controllers orchestrate, services coordinate, domain enforces rules, repositories persist

See [DECISION_LOG.md](../DECISION_LOG.md) D-019 for the full rationale and alternatives considered. See [docs/architecture-before-after.md](architecture-before-after.md) for the layered architecture diagram.
...
### 9.8 Don't Declare Business Constants in Service Classes

Business constants such as field lengths, regex patterns, and format strings **must** live in `com.cardemo.domain.constants/`, not scattered across service classes.

```java
// WRONG — local constant in a service class (duplicates across 5 files)
@Service
public class AccountUpdateService {
    private static final int ACCOUNT_ID_LENGTH = 11;  // ← DO NOT DO THIS
}

// CORRECT — centralize in domain/constants/
@Service
public class AccountUpdateService {
    // Use AccountConstants.ACCOUNT_ID_LENGTH from com.cardemo.domain.constants
}
```

If a constant you need doesn't exist in a `*Constants` class, add it there first, then reference it from the service. The constants classes are `final` with a private constructor and contain grouped `public static final` fields.

### 9.9 Don't Duplicate `DateTimeFormatter` Instances

Before the refactoring, 11 different files each created their own `DateTimeFormatter.ofPattern("yyyyMMdd")`. All shared `DateTimeFormatter` instances now live in `com.cardemo.domain.constants.DateFormatConstants`.

```java
// WRONG
private static final DateTimeFormatter CCYYMMDD_FORMATTER =
    DateTimeFormatter.ofPattern("yyyyMMdd");  // ← DO NOT RECREATE

// CORRECT
import static com.cardemo.domain.constants.DateFormatConstants.CCYYMMDD_FORMATTER;
// ... then use CCYYMMDD_FORMATTER directly
```

This applies to all `service/**` and `batch/**` classes.

### 9.10 Controllers Must Inject Service Interfaces, Not Concrete Classes

All 8 controllers inject services **by interface type** from `com.cardemo.service.interfaces/`. Do not reference concrete `*Service` or `*ServiceImpl` classes in controllers.

```java
// WRONG — concrete class injection
@RestController
public class BillingController {
    private final BillPaymentService billPaymentService;  // ← concrete class
}
...
This pattern enables mock-based unit testing of controllers and decouples the HTTP layer from service implementation details.

### 9.11 Exception Handlers Go in `GlobalExceptionHandler.java`

All `@ExceptionHandler` methods belong in `com.cardemo.config.GlobalExceptionHandler.java` (the standalone `@RestControllerAdvice` class). Do **not** add handlers to `WebConfig.java` — that class is restricted to CORS and request logging concerns only.

When adding a new handler:

1. Add the method to `GlobalExceptionHandler.java`
2. Annotate with `@ExceptionHandler(YourException.class)`
...
### 10.4 Adding to the Layered Architecture (Domain / Validation / Rules / Interfaces)

When extending CardDemo, route each type of new code to the correct package per the post-refactoring conventions (see [DECISION_LOG.md](../DECISION_LOG.md) D-019).

| Change type | Target location | Notes |
|-------------|-----------------|-------|
| New shared business constant (field length, regex, format pattern, page size) | `src/main/java/com/cardemo/domain/constants/{Domain}Constants.java` | Add to an existing domain-appropriate constants class, or create a new `final` holder class with a private constructor. Never declare in a service class. |
| New field-level validator (length, range, regex match) | `src/main/java/com/cardemo/domain/validation/{Entity}Validator.java` | Framework-independent pure Java. Throws `ValidationException` with COBOL-parity messages. |
| New business rule/calculation (formula, multi-step check) | `src/main/java/com/cardemo/domain/rules/{Rule}Rules.java` | Stateless, Spring-free, unit-testable without context. Examples: `CreditLimitRules`, `InterestCalculationRules`. |
| New service method | 1. Add to `src/main/java/com/cardemo/service/interfaces/{Domain}Service.java`<br/>2. Implement in `src/main/java/com/cardemo/service/{domain}/{ClassName}.java` | Controllers inject the interface type. |
| New service class (new domain area) | 1. Create interface `src/main/java/com/cardemo/service/interfaces/NewService.java`<br/>2. Implement `@Service class NewServiceImpl implements NewService` under `service/{new-domain}/`<br/>3. Inject `NewService` (the interface) into controllers | Follow the Service Interface Pattern from D-019. |
| New exception handler | `src/main/java/com/cardemo/config/GlobalExceptionHandler.java` (never `WebConfig.java`) | Add `@ExceptionHandler(YourException.class)` with consistent `ErrorResponse` record. |

**Testing convention:**

For each new domain class, add a corresponding unit test under `src/test/java/com/cardemo/unit/domain/` (examples include `AccountValidatorTest`, `CreditLimitRulesTest`, `InterestCalculationRulesTest`). Domain tests run fast because they do **not** load a Spring context. Service-layer tests continue to live under `src/test/java/com/cardemo/unit/service/` and exercise the orchestration logic with mocked domain collaborators.

**Why follow this pattern?**

- Keeps business rules independent of Spring annotations (faster unit tests)
- Eliminates constant duplication (single source of truth)
- Enables interface-based mocking in controller tests
- Maintains COBOL parity — only code organization changes, never behavior
