...
| 7 | [Post-Refactoring Layered Architecture](#7-post-refactoring-layered-architecture-evolution) | Evolution from flat Controller→Service→Repository to layered Controller→Service→Domain→Repository with service interfaces and decomposed config classes |
...
## 7. Post-Refactoring Layered Architecture Evolution

Sections 1-6 above document the **first** architecture transition: from z/OS mainframe to the initial Java/Spring Boot migration. This Section 7 documents a **second** architecture transition applied on top of the initial migration: the **layered domain-driven refactoring** that restructures the Java codebase without changing any externally observable behavior.

This refactoring preserves **100% behavioral parity** with both the initial Spring Boot migration and the original COBOL application. No REST endpoint paths change. No batch job outputs change. No validation rules, interest formulas, credit limit checks, or date validation cascades change. Only the **internal package organization** of the Java source code changes.

See [DECISION_LOG.md](../DECISION_LOG.md) D-019 for the full decision rationale, alternatives considered, and risks.

### 7a. Initial Spring Boot Migration Structure (Refactor-Before)

The initial Spring Boot migration uses a **flat three-layer structure**: `Controller → Service → Repository`. Service classes directly contain business rules, validation logic, data mapping, and orchestration in single monolithic classes. Some classes grew large:

- `AccountUpdateService.java` — 980 lines (mixes validation, mapping, orchestration)
- `TransactionAddService.java` — 763 lines (embedded validation cascade and format constants)
- `WebConfig.java` — 840 lines (mixes CORS config + Jackson config + 15-method `GlobalExceptionHandler` inner class)
- `DateValidationService.java` — 703 lines (procedural paragraph-mapped structure preserving COBOL parity)

Shared business constants (e.g., `ACCOUNT_ID_LENGTH = 11`, `CARD_NUM_MAX_LENGTH = 16`, `DateTimeFormatter.ofPattern("yyyyMMdd")`) were duplicated across 20+ service files. No service interfaces existed — every controller injected concrete `*Service` classes directly.
...
graph LR
    ...
    style RefactorBefore fill:#FFEBEE,stroke:#C62828,color:#B71C1C
```

### 7b. Post-Refactoring Layered Domain-Driven Structure (Refactor-After)

The refactoring introduces a **new `domain` layer** between service and repository layers, **extracts service interfaces** into a dedicated `service/interfaces/` package, and **decomposes `WebConfig.java`** into three single-responsibility configuration classes. Constants are consolidated into domain-specific holder classes. No business logic is rewritten — methods are relocated and reorganized.

```mermaid
graph LR
    subgraph RefactorAfter["Post-Refactoring — Layered Domain-Driven"]
        direction TB
        RC["@RestController<br/>8 controllers<br/>(inject interface type)"]
        RI["service.interfaces/<br/>8 service interfaces<br/>(contracts only)"]
        RS["@Service<br/>20 implementations<br/>(orchestration +<br/>delegation to domain)"]
        RD["domain/<br/>constants + validation + rules<br/>(framework-independent)"]
        RR["@Repository<br/>11 JPA interfaces"]
        RC -->|"@Autowired<br/>interface type"| RI
        RI -.->|"implemented by"| RS
        RS -->|"delegates validation<br/>+ business rules"| RD
        RS -->|"JpaRepository"| RR
        RD -.->|"framework-independent<br/>pure business logic"| RD
    end
    style RefactorAfter fill:#E8F5E9,stroke:#2E7D32,color:#1B5E20
```
...
```mermaid
graph TB
    subgraph ROOT["com.cardemo (root package)"]
        CONTROLLER["controller/<br/>8 REST controllers<br/>(inject interface type)"]

        subgraph SERVICE_PKG["service/"]
            INTERFACES["interfaces/<br/>8 service contracts:<br/>AccountService, AdminService,<br/>AuthService, BillingService,<br/>CardService, MenuService,<br/>ReportService, TransactionService"]
            IMPLS["service/{domain}/<br/>20 @Service implementations<br/>account · admin · auth · billing<br/>card · menu · report · shared · transaction"]
        end

        subgraph DOMAIN_PKG["domain/ (NEW)"]
            CONSTANTS["constants/<br/>AccountConstants<br/>CardConstants<br/>DateFormatConstants<br/>PaginationConstants<br/>TransactionConstants<br/>ValidationPatterns"]
            VALIDATION["validation/<br/>AccountValidator<br/>CardValidator<br/>TransactionValidator<br/>UserValidator"]
            RULES["rules/<br/>CreditLimitRules<br/>InterestCalculationRules<br/>TransactionPostingRules"]
        end

        REPOSITORY["repository/<br/>11 JPA interfaces<br/>(unchanged)"]

        subgraph CONFIG_PKG["config/ (DECOMPOSED)"]
            WEBCFG["WebConfig.java<br/>CORS + request logging ONLY"]
            JACKSON["JacksonConfig.java<br/>Jackson ObjectMapper<br/>customization"]
            GEH["GlobalExceptionHandler.java<br/>@RestControllerAdvice<br/>15 @ExceptionHandler methods"]
            OTHERCFG["AwsConfig · BatchConfig<br/>JpaConfig · ObservabilityConfig<br/>SecurityConfig (unchanged)"]
        end

        MODEL["model/<br/>DTOs · entities · enums · keys<br/>(unchanged — import updates only)"]
        BATCH["batch/<br/>jobs · processors · readers · writers<br/>(use domain constants + rules)"]
        EXCEPTION["exception/<br/>7 classes (unchanged)"]
        OBSERVABILITY["observability/<br/>3 classes (unchanged)"]
    end

    CONTROLLER -->|"injects"| INTERFACES
    INTERFACES -.->|"implemented by"| IMPLS
    IMPLS -->|"delegates to"| VALIDATION
    IMPLS -->|"delegates to"| RULES
    IMPLS -->|"uses"| CONSTANTS
    IMPLS -->|"calls"| REPOSITORY
    BATCH -->|"uses"| CONSTANTS
    BATCH -->|"delegates to"| RULES
    MODEL -.->|"referenced by"| IMPLS
    MODEL -.->|"referenced by"| VALIDATION

    style ROOT fill:#F5F5F5,stroke:#424242
    style CONTROLLER fill:#FFF3E0,stroke:#E65100,color:#BF360C
    style INTERFACES fill:#FFECB3,stroke:#FF8F00,color:#E65100
    style IMPLS fill:#E8F5E9,stroke:#2E7D32,color:#1B5E20
    style DOMAIN_PKG fill:#E3F2FD,stroke:#1565C0,color:#0D47A1
    style CONSTANTS fill:#BBDEFB,stroke:#1976D2,color:#0D47A1
    style VALIDATION fill:#BBDEFB,stroke:#1976D2,color:#0D47A1
    style RULES fill:#BBDEFB,stroke:#1976D2,color:#0D47A1
    style REPOSITORY fill:#F3E5F5,stroke:#6A1B9A,color:#4A148C
    style CONFIG_PKG fill:#FFF9C4,stroke:#F9A825,color:#F57F17
    style WEBCFG fill:#FFF59D,stroke:#F9A825,color:#F57F17
    style JACKSON fill:#FFF59D,stroke:#F9A825,color:#F57F17
    style GEH fill:#FFF59D,stroke:#F9A825,color:#F57F17
    style OTHERCFG fill:#FFF59D,stroke:#F9A825,color:#F57F17
    style MODEL fill:#F8BBD0,stroke:#C2185B,color:#880E4F
    style BATCH fill:#FFCCBC,stroke:#D84315,color:#BF360C
    style EXCEPTION fill:#D7CCC8,stroke:#5D4037,color:#3E2723
    style OBSERVABILITY fill:#C5CAE9,stroke:#3949AB,color:#1A237E
```

### 7d. New Layer Responsibilities

| Layer | Responsibility | Key Classes | Framework Coupling |
|-------|----------------|-------------|---------------------|
| **Controller** | HTTP request/response mapping; inject service interfaces | 8 `@RestController` classes | Spring MVC |
| **Service Interface** | Public contract for each service domain | 8 interfaces in `service/interfaces/` | None (plain Java) |
| **Service Implementation** | Orchestration: call repositories, delegate validation/rules to domain, handle transactions | 20 `@Service` classes across 9 sub-packages | Spring (`@Service`, `@Transactional`) |
| **Domain — constants** | Centralized `public static final` business constants | 6 `final` holder classes in `domain/constants/` | None (plain Java) |
| **Domain — validation** | Field-level validators (length, range, regex, cascade) | 4 validator classes in `domain/validation/` | None (plain Java) |
| **Domain — rules** | Stateless business rule calculations (formulas, multi-step checks) | 3 rule classes in `domain/rules/` | None (plain Java) |
| **Repository** | Database access via Spring Data JPA | 11 `JpaRepository` interfaces | Spring Data JPA |

### 7e. Decomposition of `WebConfig.java`

The original 840-line `WebConfig.java` mixed three unrelated concerns in one class. The refactoring extracts these into three focused classes, each with a single reason to change:

```mermaid
graph LR
    subgraph Before7e["Before — Monolithic WebConfig.java (840 lines)"]
        direction TB
        OLD["WebConfig.java<br/>• CORS config<br/>• Jackson customizer<br/>• Request logging filter<br/>• ErrorResponse record<br/>• GlobalExceptionHandler inner class<br/>(15 @ExceptionHandler methods)"]
    end
    style Before7e fill:#FFEBEE,stroke:#C62828,color:#B71C1C

    subgraph After7e["After — Three Single-Responsibility Classes"]
        direction TB
        NEW1["WebConfig.java (reduced)<br/>• CORS config<br/>• Request logging filter"]
        NEW2["JacksonConfig.java (new)<br/>• Jackson2ObjectMapperBuilder-<br/>Customizer bean"]
        NEW3["GlobalExceptionHandler.java (new)<br/>• @RestControllerAdvice<br/>• 15 @ExceptionHandler methods<br/>• ErrorResponse record (colocated)"]
    end
    style After7e fill:#E8F5E9,stroke:#2E7D32,color:#1B5E20

    OLD -->|"extract CORS +<br/>request logging"| NEW1
    OLD -->|"extract Jackson<br/>customizer"| NEW2
    OLD -->|"extract exception<br/>handler + record"| NEW3
```

| Class | Lines (approx) | Concern | Annotation |
|-------|----------------|---------|------------|
| `WebConfig.java` (reduced) | ~150 | CORS + request logging | `@Configuration implements WebMvcConfigurer` |
| `JacksonConfig.java` (new) | ~60 | Jackson ObjectMapper customization | `@Configuration` with `@Bean` |
| `GlobalExceptionHandler.java` (new) | ~520 | Error mapping to HTTP responses | `@RestControllerAdvice` |

### 7f. Service Interfaces — The 8 New Contracts

Every service domain now has a public interface in `com.cardemo.service.interfaces/`. Controllers and batch orchestrators inject the **interface type**, enabling mock-based unit testing and future alternative implementations.

| Interface | Implementations (in `service/{domain}/`) | Primary Consumer |
|-----------|------------------------------------------|------------------|
| `AccountService` | `AccountUpdateService`, `AccountViewService` | `AccountController` |
| `AdminService` | `UserAddService`, `UserDeleteService`, `UserListService`, `UserUpdateService` | `UserAdminController` |
| `AuthService` | `AuthenticationService` | `AuthController` |
| `BillingService` | `BillPaymentService` | `BillingController` |
| `CardService` | `CardDetailService`, `CardListService`, `CardUpdateService` | `CardController` |
| `MenuService` | `MainMenuService`, `AdminMenuService` | `MenuController` |
| `ReportService` | `ReportSubmissionService` | `ReportController` |
| `TransactionService` | `TransactionAddService`, `TransactionDetailService`, `TransactionListService` | `TransactionController` |

### 7g. Refactoring Impact Summary

| Aspect | Before Refactoring | After Refactoring |
|--------|-------------------|-------------------|
| **Largest service class** | `AccountUpdateService` 980 lines | Service orchestrates; validation moved to `AccountValidator` |
| **`WebConfig.java`** | 840 lines, 3 mixed concerns | Split into `WebConfig` (~150L) + `JacksonConfig` (~60L) + `GlobalExceptionHandler` (~520L) |
| **Service interfaces** | 0 (all concrete classes) | 8 interfaces, all services implement one |
| **Duplicated `DateTimeFormatter` instances** | 11 files (each with local declaration) | 1 source: `DateFormatConstants.CCYYMMDD_FORMATTER` |
| **Duplicated field-length constants** | `ACCOUNT_ID_LENGTH` in 5 files, `CARD_NUM_MAX_LENGTH` in 3 files | Single `AccountConstants` / `CardConstants` holder classes |
| **Total scattered constants consolidated** | ~67 declarations across 26 files | 24 unique constants in 6 domain holder classes |
| **Business rule extraction** | Embedded in service methods | `CreditLimitRules`, `InterestCalculationRules`, `TransactionPostingRules` |
| **Test execution for domain logic** | Requires Spring context | Pure JUnit (no context) — faster test cycle |
| **Behavioral parity** | 22 features, 100% COBOL parity | 22 features, 100% COBOL parity **preserved** |
| **REST API contracts** | 8 controllers, all endpoints | 8 controllers, all endpoints **unchanged** |
| **Test count** | 55 test files | 55 test files preserved + 5 new `unit/domain/*` tests |

### 7h. What Does NOT Change

Per AAP Section 0.3.2 (Explicitly Out of Scope) and Rule R-006 (API contract stability), the refactoring explicitly does NOT alter:

- **REST endpoint paths, HTTP methods, request/response schemas** — see [api-contracts.md](api-contracts.md)
- **All 22 features** — 18 online transactions (F-001 through F-017) + 10 batch programs
- **Business logic** — interest formula `(balance × rate) / 1200`, BigDecimal scale/rounding, date validation cascades, FICO score ranges, credit limit checks
- **Database schema** — Flyway migrations `V1`/`V2`/`V3` remain untouched
- **AWS infrastructure** — S3 buckets, SQS FIFO queue, SNS topics
- **Security** — HTTP Basic Auth, BCrypt, `SecurityConfig.java` authorization rules
- **Observability** — Jaeger tracing, Prometheus metrics, Grafana dashboards, structured logging
- **Batch pipeline stage sequence** — POSTTRAN → INTCALC → COMBTRAN → parallel STMTGEN + TXNRPT
- **Validation gates** — all 8 gates in `validation-gates.md` remain the acceptance criteria

### 7i. Cross-References

- [DECISION_LOG.md](../DECISION_LOG.md) — D-019 "Layered Domain-Driven Refactoring" (rationale, alternatives, risks)
- [README.md](../README.md) — Package Overview table (updated to include `domain.*` and `service.interfaces`)
- [onboarding-guide.md](onboarding-guide.md) — Section 7.6 "Layered Architecture — Post-Refactoring Package Organization" and Section 10.4 "Adding to the Layered Architecture"
- [TRACEABILITY_MATRIX.md](../TRACEABILITY_MATRIX.md) — All 28 COBOL program mappings remain valid; extracted domain classes inherit the COBOL traceability comments from their source service classes
