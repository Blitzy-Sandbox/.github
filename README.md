...
│   │   │   ├── config/                        # Security, Batch, AWS, JPA, Web (CORS), Jackson, GlobalExceptionHandler, Observability
│   │   │   ├── model/
...
│   │   │   │   ├── constants/                 # Centralized constants (Account, Card, Transaction, Date, Pagination, Validation)
│   │   │   │   ├── validation/                # Field validators (AccountValidator, CardValidator, TransactionValidator, UserValidator)
│   │   │   │   └── rules/                     # Business rules (CreditLimitRules, InterestCalculationRules, TransactionPostingRules)
...
│   │   │   │   └── interfaces/                # Service contracts (AccountService, CardService, etc.)
...
|:---|:---|
| `config` | Spring configuration classes — Security (BCrypt), Batch infrastructure, AWS clients, JPA, observability, web/CORS, Jackson serialization, and global exception handling (@RestControllerAdvice) |
...
| `domain.constants` | Centralized business constants — eliminates duplication across services and batch classes (Account IDs, Card formats, Date patterns, Pagination sizes, Transaction formats, Validation regex) |
| `domain.validation` | Framework-independent field validators — reusable validation logic extracted from service classes (AccountValidator, CardValidator, TransactionValidator, UserValidator) |
| `domain.rules` | Core business rules — stateless rule classes for credit limit checking, interest calculation formulas, and 4-stage transaction posting validation cascade |
| `repository` | Spring Data JPA repositories — one per VSAM dataset, with custom queries for alternate index access patterns |
| `service` | Business logic orchestration — each COBOL online program maps to one service class; services implement contracts from `service.interfaces` and delegate field validation/business rules to the `domain` layer |
| `service.interfaces` | Service contracts (interfaces) — enables constructor injection by interface type for testability and future alternative implementations (AccountService, AdminService, AuthService, BillingService, CardService, MenuService, ReportService, TransactionService) |
